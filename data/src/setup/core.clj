...
; write campaigns and ads to "comapign-ids.txt" and "ad-ids.txt"
...
; load campaign/ad ids to a map
; NB: always returns nil
(defn load-ids []
  (try
...
(defn dostats []
  ...
  (let [json-string-mapper      (map json/parse-string)
        ...
        campaign-buckets-mapper (map
                                 (fn camp-buck-map [event]
...
; get status from generated data
...
; insert ad-campaign k/v pair to redis, and return all the ad list
; totally num-campaign * 10 ads
...
; generate kafka record string
(defn make-kafka-event-at [time with-skew? ads user-ids page-ids]
  (let [ad-types    ["banner", "modal", "sponsored-search", "mail", "mobile"]
        event-types ["view", "click", "purchase"]
        skew        (if with-skew? (- 50 (rand-int 100)) 0)
...
; runs, which sends records to kafka
...
; flush redis and write campaigns seqs to it
; redis ops: SADD key [vals], SMEMBERS key
...
(defn get-conf [confPath]
  (let [conf        (yaml/parse-string (slurp confPath))
        ...
        kafka-hosts (clojure.string/join
                     (interpose ","
                                (for [broker (get conf :kafka.brokers)]
...
(def cli-options
  [["-s"
    "--setup"
    "Set up for catchup-simulation-mode (or re-setup if the .txt files exist)"]
   ["-c"
    "--check"
    "Check that the catchup-mode data has been properly processed by whatever is being benchmarked."]
   ["-n"
    "--new"
    "Set up redis for a new real-time simulation. This must be run on only one node,"
    " only once before starting multiple kafka-feeders (-r) on multiple nodes."]
   ["-r"
    "--run"
    "Run - emit events to kafka at a particular frequency This is used for the real-time simulation."
    "Frequency specified with other options."]
   ["-t"
    "--throughput COUNT"
    "Should be used with '-r'. This is the number of tuples per second to emit."
    "(Obviously it can't emit ridiculous numbers per second.)"
    :default  0
    :parse-fn #(Long/parseLong %)]
   ["-w" "--with-skew" "Add minor skew and late tuples into the mix."]
   ["-g"
    "--get-stats"
    "Read through redis and collect stats on end-to-end latency and so forth for the real-time simulation."]
   ["-a"
    "--configPath PATH"
    "Path to config yaml file"
    :default  "../conf/epccConf.yaml"
